...
public class SearchInSortedMatrix {
    ...
    private static boolean binarySearcInMatrix(int[][] matrix, int target){
        int row_num = matrix.length; // Take number of rows and store them in a variable
        int col_num = matrix[0].length;// Take number of columns and store them in a variable
        int start = 0,end = row_num*col_num-1; // Take start as 0 and end as row*col-1
        while (start<=end){// run the loop till start is less than end
            int mid = start+(end-start)/2; // take the mid-index
            int midValue = matrix[mid/col_num][mid%col_num];// Take the mid index value

            if (midValue==target){
                return true;// If mid value == target return true
            }
            else if (midValue<target){
                start=mid+1;// else shift right
            }
            else end=mid-1;// shift left
        }
        return false;
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(binarySearcInMatrix(matrix,3));
    }
}
